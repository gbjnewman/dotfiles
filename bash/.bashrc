...
PROMPT_BEFORE="${txtcyn}\u@\h ${txtwht}\w${txtpur}"

# If SSH session, prepend (ssh session) and IP in red
if [ -n "$SSH_CONNECTION" ]; then

	ssh_ip=$(ip -4 addr show scope global | awk '/inet / {print $2}' | cut -d/ -f1 | head -n 1)
	PROMPT_BEFORE="${txtred}(ssh session) ($ssh_ip) ${PROMPT_BEFORE}"
fi
...
if [ "$color_prompt" = yes ]; then
   ...
   # Git-aware prompt
   # sync history and build git aware prompt
   __update_prompt() {
	   history -a
	   history -n
	   __git_ps1 "$PROMPT_BEFORE" "$PROMPT_AFTER"
   }
